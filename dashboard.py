...
def get_creds_path():
    # Scriptin bulunduğu klasöre göre mutlak yolu belirler
    base_dir = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(base_dir, "credentials", "gcp-key.json")

# ── BIGQUERY CLIENT (Güvenli Bağlantı) ────────────────────────────────────────
@st.cache_resource
def get_client():
    path = get_creds_path()
    ...
    # credentials yoksa default (st.secrets veya env vars) kullanır
...
if page == "🏠 Executive Summary":
    ...
    df = safe_query(f"SELECT * FROM `{PROJECT}.{DATASET}.mart_gold_monthly_executive_metrics` ORDER BY year_month")
    
    if not df.empty:
        # API EXPORT (Sidebar'da)
        csv = df.to_csv(index=False).encode('utf-8')
        st.sidebar.download_button("📥 Dataseti CSV İndir", csv, "epias_summary.csv", "text/csv")
        ...
        st.plotly_chart(px.line(df, x="year_month", y="avg_ptf", title="Aylık Trend", template="plotly_dark"), use_container_width=True)

        # ML BACKTESTING & SHAP
        st.markdown("---")
        st.subheader("🎯 Model Başarımı (2026 Saatlik)")
        ...
        with c_bt:
            ...
            if not df_bt.empty:
                fig = go.Figure()
                fig.add_trace(go.Scatter(x=df_bt["dt"], y=df_bt["a_ptf"], name="Gerçek", line=dict(color="#00d4ff")))
                fig.add_trace(go.Scatter(x=df_bt["dt"], y=df_bt["p_ptf"], name="Tahmin", line=dict(color="#ff6b35", dash="dash")))
        ...
        with c_sh:
            try:
                ...
                st.plotly_chart(px.bar(shap_df.head(10), x='feature_importance_vals', y='col_name', orientation='h', title="SHAP Karar Yapısı", template="plotly_dark"), use_container_width=True)
            except: st.info("SHAP dosyası bulunamadı.")
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    if not df.empty:
        st.plotly_chart(px.scatter(df, x="ptf", y="smf", color="system_direction", title="PTF vs SMF", template="plotly_dark"), use_container_width=True)
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    if not df.empty:
        st.plotly_chart(px.scatter(df, x="renewable_ratio", y="ptf", color="ptf", title="Merit Order Etkisi", template="plotly_dark"), use_container_width=True)
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    if not df.empty:
        ...
        st.plotly_chart(px.bar(tod, x="time_of_day", y="coverage_ratio", title="Ortalama Karşılama Oranı", template="plotly_dark"), use_container_width=True)
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    if not df.empty:
        st.plotly_chart(px.line(df, x="hour", y=["forecast_consumption", "actual_consumption"], title="Tahmin (LEP) vs Gerçekleşen", template="plotly_dark"), use_container_width=True)
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    if not df.empty:
        # MEVSİMSEL TABLO (pd.Series ve x.name Düzeltmesi)
