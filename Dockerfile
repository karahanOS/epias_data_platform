...
# 3. Spark indir ve kur
#    NOT: ADD satırı BURADAN ÖNCE çalıştırılmamalı —
#    /opt/spark henüz yok, önceden kopyalanırsa mv komutu
#    Spark'ı yanlış dizine yerleştirir.
...
# 4. GCS connector JAR — tek bir versiyonlu shaded JAR yeterli.
#    Önceki ADD satırı kaldırıldı: iki farklı JAR classpath'te
#    class çakışmasına (ClassCastException / NoSuchMethodError) yol açıyordu.
...
# 5. Yetki ver ve symlink oluştur
RUN chmod -R 755 /opt/spark && \
    ln -sf /opt/spark/bin/spark-submit /usr/bin/spark-submit

# 6. /home/airflow izinlerini düzelt (pip izin hatasını önler)
...
# 7. Python paketleri — _PIP_ADDITIONAL_REQUIREMENTS'a gerek kalmaz
