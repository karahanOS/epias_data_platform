...
**Status:** Accepted — fully implemented 2026-07-26 (all 10 action items done)
...
10. [x] Set a GCP budget alert (2026-07-26): ₺1500/month threshold on the `epias-data-platform` project, default 50/90/100% email notification thresholds. Sized against a real first-week Billing Report read (₺119.14 for 19-25 July), most of which (~₺90, Cloud Storage + Dataproc) was one-time remediation cost from that week's dedup/testing work, not steady-state — Compute Engine (the VM, the one genuinely recurring line item) was only ₺9.51 for a partial day. Revisit this threshold once a "clean" week of steady-state billing data is available.
